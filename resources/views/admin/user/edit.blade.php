        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-4">
                <div class="flex items-center gap-3">
                    ...
                    <h1 class="text-xl font-bold">Admin Panel - Edit User</h1>
                </div>
            ...
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <select name="role" id="role" required
                        ...
                        <option value="user" {{ old('role', $user->role) == 'user' ? 'selected' : '' }}>🛍️ User</option>
                    </select>
                    @error('role')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <textarea name="alamat" id="alamat" rows="3"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 focus:border-transparent @error('alamat') border-red-500 @enderror">{{ old('alamat', $user->alamat) }}</textarea>
                    @error('alamat')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
